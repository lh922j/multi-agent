...
- RAGAgent가 "일반 지식 기반"으로 작성한 답변을 전달했으면: 그 내용을 정리해서 사용자에게 답변하세요. 정보가 없다고 안내하지 마세요.
- 이전 에이전트 메시지에 "정보가 없습니다"만 있고 지역 정보 내용이 전혀 없을 때만: "해당 지역/조건의 데이터를 찾을 수 없습니다"라고 안내하세요.
