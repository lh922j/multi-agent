...
|--------|-----------|----------|------|
| `multiturn-04` "그 중 음식점은 몇 개야?" | 0.09 | DeepEval이 멀티턴 맥락 없이 단독 평가 → "그 중"의 지시 대상 불명으로 질문-답변 무관 판정 | 평가 메트릭 한계 |
| `rag-01` 마포구 유사 지역 | 0.29 | 번호 목록 형식 + retrieval context 없음 → AnswerRelevancyMetric 저평가 | 평가 메트릭 편향 |
| `trade-03` 강남역 근처 전세 | 0.33 | DeepEval이 한국어 테이블 답변을 "관련 없는 문장 다수"로 파싱 | 평가 메트릭 편향 |
| `anomaly-06` 직거래 비율 높은 지역 | 0.42 | 번호 목록 형식 답변 → AnswerRelevancyMetric 저평가 (GEval=1.0으로 실제 품질은 정상) | 평가 메트릭 편향 |
| `commercial-05` 홍대 주점 개수 | 0.44 | "몇 개야" 단답 질문에 목록 테이블 포함 → 부가 정보를 노이즈로 판정 | 평가 메트릭 편향 |

GEval(사람 기준 종합 평가) 기준으로는 같은 케이스들이 모두 **0.88~1.0**으로 정상 범위입니다.  
즉 실제 답변 품질은 양호하지만, AnswerRelevancyMetric이 멀티턴 맥락 누락·한국어 테이블·목록 형식에 편향된 저평가를 보이는 메트릭 특성이 주된 원인입니다.
