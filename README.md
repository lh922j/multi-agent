...
**선택 근거**: "강남구 이상거래 탐지하고 학군도 알려줘"처럼 여러 에이전트가 협력해야 하는 복합 질문에서, 그래프 엣지로 모든 경우의 수를 사전 정의하는 방식보다 Handoff 패턴이 더 유연하게 동작합니다. 프로덕션 레퍼런스가 적다는 단점은 인지하고 있으며, 실제로 API 변경으로 `autogen-agentchat==0.4.9.3` 버전을 고정해 사용합니다.
...
골든셋 22건 기준 자동 평가 (DeepEval GEval + 시스템 지표).

### 핵심 지표 요약

| 지표 | 결과 | 설명 |
|------|------|------|
| **라우팅 정확도** | **100%** | 22건 전체 올바른 에이전트로 라우팅 |
| **GEval (LLM-as-Judge)** | **0.893 / 1.0** | 답변 관련성·완결성 종합 |
| **Faithfulness** | **0.831 / 1.0** | DB 데이터 기반 사실성 |
| **Map Points 정확도** | **90.9%** | 지도 마커 반환 여부 |
| **수치 정확도** | **76.9%** | 가격·면적 수치 일치율 |
| **평균 응답 시간** | **6.73초** | ReportAgent 제거 후 34% 개선 (기존 10.25초) |
| **범위 외 거부 응답** | **0.2초** | LLM 없이 키워드 규칙만으로 즉시 거절 |
...
```bash
# 평가 실행
python tests/eval/run_eval.py              # 전체 56건
pytest tests/eval/test_system_metrics.py  # 라우팅·latency (CI)
```

### 수치 정확도 실패 분석 및 개선

초기 평가(22건)에서 수치 정확도 76.9%가 나왔으며 3건이 실패했습니다.

| 케이스 | 실패 원인 | 유형 | 조치 |
|--------|-----------|------|------|
| trade-02 | 골든셋 reference 범위가 실제 데이터보다 좁음 | reference 오류 | reference 수정 |
| commercial-03 | 편의점 질문에 카페 데이터가 reference로 등록됨 | reference 오류 | reference 수정 |
| prediction-01 | 예측 모델이 강남구 디폴트 좌표에서 낮은 값 반환 | 모델+질문 모호성 | 질문 구체화 |

**구조적 원인**: `query_trade_data`가 개별 거래 목록만 반환하면 LLM이 요약 시 숫자를 자의적으로 변형합니다.

**개선 조치**:
1. `query_trade_data`, `query_rent_data` 반환값에 `▶ 평균 X억원 | 최저 Y억원 | 최고 Z억원` 집계 통계 고정 추가
2. DataQueryAgent 프롬프트에 "도구가 반환한 수치를 그대로 인용, 변형 금지" 규칙 추가
3. 골든셋 22건 → 56건 확장 (멀티턴 2→6건, 이상거래 2→6건, 예측 3→6건)
